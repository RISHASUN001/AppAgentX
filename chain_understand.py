...
for attempt, uri in enumerate(alternative_uris, 1):
    try:
        print(f"[Chain Understand] Attempt {attempt}: Trying URI {uri}")
        db = Neo4jDatabase(uri, AUTH)
        print(f"[Chain Understand] Successfully connected to Neo4j using {uri}")
        break
    except Exception as e:
        print(f"[Chain Understand] Attempt {attempt} failed with URI {uri}: {e}")
        if attempt == len(alternative_uris):
            print("[Chain Understand] All connection attempts failed.")
            raise
